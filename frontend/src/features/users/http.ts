// Transport layer for the User Management API: a single `callJson` helper that
// attaches the Bearer token, normalizes empty 2xx bodies to `undefined`, and turns
// API-05 error envelopes into `UsersApiError`.
...
export async function callJson<T>(path: string, opts: CallOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new UsersApiError(response.status, envelope)
  }
