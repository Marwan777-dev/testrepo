// Transport layer for the persona-baselines API: a single `callJson` helper that
// attaches the Bearer token, normalizes empty 2xx bodies to `undefined`, and turns
// API-05 error envelopes into `PersonaBaselinesApiError`.
...
const API_BASE = "/api/v1/persona-baselines"
...
/**
 * Performs a JSON request against the persona-baselines API.
 *
 * - Sends `Authorization: Bearer <token>` when a session token is present.
 * - Treats 204, and any 2xx with an empty body, as `undefined`.
 * - On non-2xx, parses the API-05 envelope (if any) and throws `PersonaBaselinesApiError`.
 */
export async function callJson<T>(path: string, opts: CallOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new PersonaBaselinesApiError(response.status, envelope)
  }
