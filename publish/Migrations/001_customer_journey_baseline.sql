...
-- -----------------------------------------------------------------------------
-- scoring_configs — strategic scoring model per journey (1:1)
-- -----------------------------------------------------------------------------
CREATE TABLE scoring_configs (
    scoring_config_id    uuid          PRIMARY KEY,
    journey_id           uuid          NOT NULL REFERENCES journeys (journey_id) ON DELETE CASCADE,
    model_type           varchar(64)   NOT NULL,
    stage_weight_mode    varchar(32)   NOT NULL DEFAULT 'Equal',
    normalization_params jsonb         NULL,
    created_at           timestamptz   NOT NULL,
    updated_at           timestamptz   NOT NULL,
    CONSTRAINT uq_scoring_configs_journey_id UNIQUE (journey_id)
);
